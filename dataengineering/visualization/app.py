...
from data.stores.stores import Store
from data.menuitems.menuitems import MenuItems
from data.locations.locations import Locations
...
def FUN_menus(zip):
    ...
    categories = menuService.findAllMenuCategoriesInStore(1)
    concCat = []
    for cat in categories:
        concCat.append({"id":cat.replace(" ","_"), "name":cat})
    print(categories)
    menus = storeService.findAllMenuItemsForStore("Fazoli's", zip)
    for menu in menus:
        menu.catid = menu.category.replace(" ","_")
        menu.recommendations = storeService.findAllRecommendationsForItem(menu.id, zip)
    return render_template("menu.html", menus = menus, zip = zip, 
        categories = concCat, enumerate=enumerate)

@app.route("/recommend/<zipcode>/<itemid>")
def FUN_recommendation(zipcode,itemid):
    storeService = Store()
    recommendations = storeService.findAllRecommendationsForItem(itemid, zipcode)
    return render_template("recommendation.html", recommendations = recommendations, zip = zip)
