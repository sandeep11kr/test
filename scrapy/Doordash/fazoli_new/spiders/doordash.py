...
class DoordashSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for allItems in menuCategories:
            title = allItems['title']       # Category
            if 'Meal' in title: continue    # Ignore Category having "Meal" word
            ...
            for item in items:
                ...
                tup1 = (itemId, title, desc, prices, calorie)
    ...
    def recom_method(self, response):
        ...
        sItemStr = "; ".join(sItemList)           # convert list to string

        # code for actual Item name

        optionCat = content['data']['menuItem']['options']
        extraCat = content['data']['menuItem']['extras']    # give parent ID for option
        idList = []
        for index in extraCat:
            if index['selectionMode'] == 'single_select':
        ...
        reqId = idList[0] if idList else None           # 1st required field id

        for index in optionCat:
            parentId = index['parentExtraId']
            # if parentId is idList:              # Uncomment for all required groups
            if parentId == reqId:
                tupList.append((parentId, index['name']))
        
        for key, value in tupList:
            tupDict.setdefault(key, []).append(value)

        group = [tupDict[key] for key in tupDict]
        print('NOS: ', group)

        
        if len(group)>0: 
            group = [item for sublist in group for item in sublist]
            # groupStr = ";".join(group)    #this for all required fields

            # combo = itertools.product(*group) # all possible combination
            # group = [' & '.join(tup) for tup in combo]
        
        #print('NOS: ', idList[0], group)
        if len(group) == 0 or iname == 'New York Style Cheesecake':
            itemName = iname
            self.s_DF = self.s_DF.append({'ID': itemId, 'Name': itemName, 'Suggested Item': sItemStr}, ignore_index= True)
        else: 
            for option in group:            # This loop is for 1st required field
                itemName = iname+'=>'+option
                self.s_DF = self.s_DF.append({'ID': itemId, 'Name': itemName, 'Suggested Item': sItemStr}, ignore_index= True)
            #itemName = iname+':'+groupStr
            
            #for elem in group:
            #    itemName = iname+':'+elem
            # self.s_DF = self.s_DF.append({'ID': itemId, 'Name': itemName, 'Suggested Item': sItemStr}, ignore_index= True)
        
        mergeDF = self.s_DF.merge(self.fazoli_DF, how= 'left')
        ...
        zip = re.findall(r'[\d]{5,}', self.address)[-1]
        
        path = os.path.join(self.path, 'doordash')
        if not os.path.exists(path): os.mkdir(path)
        path = os.path.join(path, 'Fazoli_DoorDash_{0}_{1}.csv'.format(self.city, zip))
        with open(path, 'w', newline= '\n', encoding= 'utf8') as f:
            f.write(self.address+'\n')
            f.write('Timing: '+self.storeTiming+'\n\n')
